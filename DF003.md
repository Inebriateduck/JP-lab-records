...
Running mWRAP post cleanup on every folder. The following script moves through all the output folders from the previous step and pulls the best bins. The amount of jobs it can spin up is adjustable, but going over 10 is not advised since it eats up cluster resources. 
